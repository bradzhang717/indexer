			...
			for _, b := range yesterdayStat {
				if b.Chain == a.Chain {
					if a.AddressCount == 0 || b.AddressCount == 0 {
						...
						chain24HourStat.Address24hPercent = float32(a.AddressCount) / float32(b.AddressCount) * 100
					}
					...
					if partA == 0 || partB == 0 {
						...
						chain24HourStat.Balance24hPercent = float32(partA) / float32(partB) * 100
					}
					if a.InscriptionsCount == 0 || b.InscriptionsCount == 0 {
						chain24HourStat.Tick24hPercent = 0
					} else {
						chain24HourStat.Tick24hPercent = float32(a.InscriptionsCount) / float32(b.InscriptionsCount) * 100
					}
